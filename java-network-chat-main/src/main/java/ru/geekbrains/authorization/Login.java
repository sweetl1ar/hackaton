        ...
        buttonLogin.addActionListener(e -> {
            try {
                if (authenticate(getNickname(), getPassword())) {
                    JOptionPane.showMessageDialog(Login.this, "Вы успешно залогинились.", "Имя пользователя", JOptionPane.INFORMATION_MESSAGE);
                    succeeded = true;
                    dispose();
                } else {
                    JOptionPane.showMessageDialog(Login.this, "Неправильные имя пользователя или пароль.", "Имя пользователя", JOptionPane.ERROR_MESSAGE);
